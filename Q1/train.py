...
def main():  
    ...
    for ep in tqdm(range(max_episodes), desc="Training eps"):
        ...
        for t in range(max_steps):
            ...
            s_, r, term, trunc, _ = env.step(a_np)
            done = term or trunc
    ...
    for _ in range(200):
        with torch.no_grad():
            a, _ = actor.sample(torch.tensor(s, dtype=torch.float32, device=device).unsqueeze(0))
        s, r, done, trunc, _ = env.step(a.cpu().numpy().flatten())
        if done or trunc:
            break
